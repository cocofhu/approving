...
export const NODE_DEFS: Record<NodeType, NodeTypeDef> = {
  ...
  app_preview: {
    ...
    fields: [
      ...
      { key: 'form', label: 'nodes.app_preview.fields.form.label', type: 'form' },
    ],
    outputs: [
      { key: 'action', desc: 'nodes.app_preview.outputs.action.desc' },
      { key: 'preview_issues', desc: 'nodes.app_preview.outputs.preview_issues.desc' },
      { key: 'reviewer_id', desc: 'nodes.app_preview.outputs.reviewer_id.desc' },
    ],
    defaults: {
      max_rounds: 3,
      title: '应用预览',
      output_var: 'action',
      actions: [{ id: 'pass', label: '通过' }, { id: 'fail', label: '退回' }],
      prompt: '在沙箱内构建并启动上游实现的应用,调用 set_preview(port, label?) 注册预览端口,供人工在 iframe 中验收。',
    },
