...
<template>
  <section class="flex h-full min-h-0 flex-col" data-testid="integrations-panel">
    <header class="mb-5 shrink-0">
      ...
      <h2 class="text-lg font-semibold text-txt">
        {{ selected ? selected.name : t('mcp.integrations.title') }}
      </h2>
      <p v-if="!selected" class="text-sm text-txt3">{{ t('mcp.integrations.sectionTitle') }}</p>
    </header>

    <div class="min-h-0 flex-1 overflow-y-auto" data-testid="integrations-panel-scroll">
      <div v-if="!selected" data-testid="integrations-panel-catalog">
      ...
      <div v-else data-testid="integrations-panel-detail" class="space-y-5">
        <div>
          <div class="flex flex-wrap items-center gap-2">
          ...
          <p v-if="isProjectScope" class="mt-3 text-[12px] leading-5 text-txt3">
          ...
          <p v-else-if="isAgentScope" class="mt-3 text-[12px] leading-5 text-txt3">
            {{ t('mcp.integrations.entryPathLabel') }}
            <span class="pointer-events-none cursor-default border-b border-dotted border-line-strong font-medium text-txt">
