...
vi.mock('@/lib/run/useWorkflowAskInputs', () => ({
  useWorkflowAskInputs: () => {
    // eslint-disable-next-line @typescript-eslint/no-require-imports -- mock factory runs before ESM imports
...
describe('WorkflowApiTab interactions', () => {
  ...
  it('loads, creates, copies and revokes API keys', async () => {
    ...
    expect((w.vm as any).copied).toBe('newkey')
    expect(mocks.success).not.toHaveBeenCalledWith(expect.stringMatching(/已复制|Copied/))
