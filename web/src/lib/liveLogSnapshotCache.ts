import type { AcpEvent, McpCall } from './types'
...
export function snapshotCacheKey(runId: string, nodeId: string): string {
...
export function clearLiveLogSnapshotsForRun(runId: string): void {
  const prefix = `${runId}\0`
  for (const k of [...store.keys()]) {
    if (k.startsWith(prefix)) store.delete(k)
  }
...
export function eventsFromSnapshot(snap: LiveLogNodeSnapshot | undefined): AcpEvent[] {
