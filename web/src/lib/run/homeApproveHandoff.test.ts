...
describe('homeApproveHandoff', () => {
  ...
  it('consumes the same run even when the parked node id differs from the guess', () => {
    setHomeApproveHandoff({
      runId: 'run-1',
      nodeId: 'ap',
      text: '做登录',
    ...
    const got = consumeHomeApproveHandoff('run-1', 'approve_7gl8')
    expect(got?.text).toBe('做登录')
    expect(got?.nodeId).toBe('approve_7gl8')
    expect(got?.images).toEqual([{ mimeType: 'image/png', data: 'abc', name: 'a.png' }])
    expect(peekHomeApproveHandoff()).toBeNull()
  })
