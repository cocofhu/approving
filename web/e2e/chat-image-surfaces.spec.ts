...
test.describe('chat image preview surfaces (pm / feedback / para / tester / shared)', () => {
  ...
  test('five previewable entries + load-failed placeholder', async ({ page }) => {
    ...
    await page.getByTestId('shared-fail-thumb').click()
    await expect(page.getByTestId('shared-image-preview-failed')).toBeVisible({ timeout: 10_000 })
    await expect(page.getByTestId('shared-image-preview-failed')).toContainText('图片加载失败')
    await page.keyboard.press('Escape')
    await expect(page.getByTestId('shared-image-preview-failed')).toBeVisible()
