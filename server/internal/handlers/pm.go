...
// ClearPmThreadContext handles DELETE /api/projects/:id/pm/threads/:tid/context.
// Wipes transcript messages (and channel ledger pointers) but keeps the thread
// row — the path QQ Channel needs, since deleting the thread itself is rejected.
func (h *Handlers) ClearPmThreadContext(c *gin.Context) {
	if h.Pm == nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "pm unavailable"})
		return
	}
	...
	t, err := h.Pm.GetThread(projectID, tid, user)
	if err != nil {
		writePmErr(c, err)
		return
	}
	if t.SandboxRef != "" {
		if id, e := strconv.ParseUint(t.SandboxRef, 10, strconv.IntSize); e == nil && h.Sbx != nil {
			if err := h.Sbx.Destroy(c.Request.Context(), uint(id)); err != nil {
				log.Warn().Err(err).Uint("sandbox", uint(id)).Msg("destroy thread sandbox on clear context failed")
			}
		}
	}
	if h.PMMCP != nil {
	...
	cleared, err := h.Pm.ClearThreadContext(projectID, tid, user)
	if err != nil {
		writePmErr(c, err)
		return
	}
	var tasksCancelled int
	if h.TaskContext != nil && cleared.ConversationID != "" && cleared.Channel != "" {
		n, cerr := h.TaskContext.ClearConversationLedger(projectID, cleared.Channel, cleared.ConversationID)
		if cerr != nil {
			log.Warn().Err(cerr).Str("thread", tid).Msg("clear conversation ledger failed")
		} else {
			tasksCancelled = n
		}
	}
	h.recordAudit(services.AuditRecord{
		ProjectID: projectID, Actor: h.auditActorFromContext(c),
		Action: models.AuditActionProjectConfig, ResourceType: "pm_thread",
		ResourceID: tid, Outcome: models.AuditOutcomeOK,
		Summary: "clear thread context",
		Payload: map[string]any{
			"messagesCleared": cleared.MessagesCleared,
