...
const (
	...
	DefaultApproveOpenSuffix       = "\n\n这是一次多轮 ReAct 对话:**第一回合必须调用 ask_question** 提出需要拍板的关键问题并等待回复。不要在开场轮给出最终结论,不要跳过提问直接 set_* / node_complete。"
	DefaultApproveOpenRetry        = "【必须提问】Approve 节点第一回合必须调用 `ask_question` 向人提出需要拍板的问题,然后立即结束本轮。现在立刻调用 `ask_question`,不要写产物、不要 node_complete。"
