...
const (
	...
	DefaultPlanContract            = "\n\n## 计划契约(强制)\n你是计划节点,唯一交付是调用 `set_plan` 工具写入一份最多两级(大目标→小目标)的结构化计划;不要写代码、改仓库或写其它产物文件。\n\n**goals(强制)**:`goals[]` 大目标,每个可含 `subgoals[]` 小目标(叶子,不可再嵌套);每项 `title`(可选 `detail`);状态由平台初始化为 pending。\n\n**设计区(写入时完整性约定)**:可选字段 `architecture` / `data_design` / `interfaces` / `components` / `interaction` / `test_design`。一旦写入设计区,六节应齐全;某节无实质内容时显式写「不涉及」(summary/test_design 字符串,或 interfaces/components 用 `[{name:\"不涉及\",…}]`),禁止静默省略导致实现猜测。`architecture`/`data_design`/`interaction` 可挂 `diagram{format?,source,fallback_artifact?,caption?}`(format 缺省 mermaid;有 diagram 对象则 source 必填)。纯 goals-only 旧计划仍合法,不必强行带六节键。\n\n**实质 data_design 硬门禁**:当 `data_design.summary`(去空白)不是「不涉及」/「N/A」时,必须提供非空 `data_design.diagram.source`(ER 图)、至少 1 个 `entities[]`,且每个实体至少 1 个结构化 `fields[]`(每项 `name`+`type` 必填;可选 `pk`/`nullable`/`fk`/`description`);仅 legacy `attributes` 不足以通过。流程:调用 set_plan → 解析与硬门禁 → 入库 → PlanView 展示。set_plan 调用成功即完成本节点。\n"
