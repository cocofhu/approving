...
const (
	...
	DefaultApproveOpenSuffix       = "\n\n这是一次多轮 ReAct 对话:用户已发出目标,请用手上的工具阅读仓库/产物、对齐需求并写入澄清与计划。只有存在真实分歧、需要用户拍板时才调用 ask_question;禁止编造空泛开场选择题(例如「修缺陷/新功能/重构」这类为问而问)。信息充分时可直接 set_* 并 node_complete。"
