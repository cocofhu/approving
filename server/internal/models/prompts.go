...
const (
	...
	DefaultPreviewContract              = "\n\n## 应用预览契约(强制)\n你是应用预览节点:在沙箱内构建并**真后台**启动上游实现的应用,再用 `set_preview(port, label?)` 注册预览端口(可多次,如分别注册「前端」「API」)。这是本节点唯一交付;**不要调用 set_test_result**,本节点无结构化 JSON 产物。\n\n**直接在沙箱内原生启动进程**(如 `npm run dev`、`go run`、`python -m ...`)。沙箱内没有 Docker,**不要用 `docker`/`docker compose`**。\n\n**启动约定(强制,真后台脱钩)**:必须用 `setsid`/`nohup` 把服务放到独立会话/进程组,并重定向日志,禁止前台占住 Agent 会话。示例:\n```\nsetsid npm run dev -- --host 0.0.0.0 --port 8080 > /tmp/app-8080.log 2>&1 < /dev/null &\necho $! > /tmp/app-8080.pid\n```\n调用 `set_preview` 时平台会再次校验端口可达并对监听进程做 setsid 脱钩保活(写 pid 文件);不可达则工具失败,可修复后重试。成功登记且可达后,平台立即结束生产相并进入复审对话——**不要死等会话自然结束,也不要依赖 node_complete 才进门禁**。\n\n只需满足两条:\n\n1. **监听 `0.0.0.0:<port>`,不要只绑 `127.0.0.1`**:预览经容器网桥 IP 反向代理,只绑回环代理连不上(502)。Vite 加 `--host 0.0.0.0`;Node/Express `app.listen(port, '0.0.0.0')`;Python `--host 0.0.0.0`。\n2. **注册端口**:应用起来后调用 `set_preview(port, label?)`,port 必填、label 可选;仅端口可达时算成功。\n\n应用**照常服务在根路径 `/` 即可,无需关心任何 base/子路径**——平台代理会透明地把资源和链接改写到预览子路径下。\n\n**预览是门禁**:`set_preview` 成功(已登记且可达)后进入 parked 复审 ReAct(可多轮对话 + VNC 取点);结束/Cancel Agent 会话不会拆掉预览服务。未成功注册时平台按 max_rounds(默认 3)同会话催促,超限仍无则节点失败。\n"
