...
type ReactTurn struct {
	...
	// AgentSummary is an optional unified induction of all human feedback in
	// this node execution, parsed from the confirm-turn dual-write output.
	// Distinct from Msg (transcript narration). Only force(confirm) turns
	// persist a non-empty value; middle turns leave it empty. Callers must not
	// fall back to FeedbackSummary / first line / bubble text.
	AgentSummary string
