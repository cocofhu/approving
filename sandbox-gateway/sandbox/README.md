...
| `PREVIEW_PICK_SCRIPT_URL` | 自动注入时为 `/__approving/preview-pick.js` | Agent 兜底写入 HTML 的 script src。注入层本身始终提供同域该路径，不使用 Approving 的 `localhost` 绝对地址 |
