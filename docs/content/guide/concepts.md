...
- 管理面板默认掩码展示；复制写入完整 URL。同一浏览器标签刷新后仍可再复制同一有效链。重新生成立刻作废旧链并沿用原档位；立即撤销后链接不可用。门禁仍 pending 时，已撤销/已过期可再创建；审批已完成后只读，不能再创建。`proposal_select` / 澄清没有此入口（应用预览走下方「待复审临时链接」同策略）。
...
### 待复审临时链接（Inbox kind=review / app_preview）

Inbox **待复审**与 **应用预览**卡片使用同一套管理面板与令牌规则（`ShareLinkKindReview`），认证 API 走 `/api/runs/:id/reviews/:nodeId/share-link*`，不复用 `/gates/...`，也不伪造 Gate 行。站内入口：卡片「复制临时链接」、应用预览工作台工具栏「分享审批」、右侧 Agent 区「打开分享面板」。公开页标识为「外部复审」：热态可多轮 ReAct（发送 / 流式轮询 / 取消）；`productKind=app_preview` 时产物区为只读占位说明，**不提供 noVNC / 取点**（登录态同一项仍可远程桌面与取点）。底栏仅「确认并流转」（无驳回、无姓名意见）。运行详情复审 Tab / 登录侧复审面板不提供临时链接入口；`proposal_select` / 澄清没有此入口。
