...
Inbox **待复审**卡片使用同一套管理面板与令牌规则，但认证 API 走 `/api/runs/:id/reviews/:nodeId/share-link*`，不复用 `/gates/...`，也不伪造 Gate 行。公开页标识为「外部复审」，三区与 human_gate 相同：热态可多轮 ReAct（发送 / 流式轮询 / 取消 / 视觉取点），底栏仅「确认并流转」（无驳回、无姓名意见）。运行详情复审 Tab / 登录侧复审面板不提供临时链接入口。
